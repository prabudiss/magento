...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function saveOrderAction()
    {
        ...
        foreach ($items as $item) {
            $cart_products[] = $item->getProductId();
        }        
        ...
        try {
            ...
            for ($i = 0; $i < count($cart_products); $i++) {
                $_product = Mage::getModel('catalog/product')->load($cart_products[$i]);
                $db->query("Insert into custom_ordered_products('id','order_id','prod_id','validity','ordered_on') VALUES('','".$order_id."','".$cart_products[$i]."','".$_product->getValidity()."','NOW()')");
            }
